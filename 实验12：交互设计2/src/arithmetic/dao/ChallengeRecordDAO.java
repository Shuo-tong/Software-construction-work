...
public class ChallengeRecordDAO {
    ...
    /** 插入一条挑战记录 */
    public boolean insert(String userId, Difficulty difficulty,
                          int total, int correct, int wrong,
                          int scoreEarned, long timeUsedMs) {
        String sql = "INSERT INTO challenge_records(user_id, difficulty, total, correct, wrong, score_earned, time_used_ms) "
                   + "VALUES(?,?,?,?,?,?,?)";
        ...
        try {
            ...
            ps.setInt(4, correct);
            ps.setInt(5, wrong);
            ps.setInt(6, scoreEarned);
            ps.setLong(7, timeUsedMs);
    ...
    public List<ChallengeRecord> findByUserId(String userId) {
        String sql = "SELECT id, user_id, difficulty, total, correct, wrong, score_earned, time_used_ms, created_at "
    ...
    public static class ChallengeRecord {
        ...
        public ChallengeRecord(long id, String userId, Difficulty difficulty,
                               int total, int correct, int wrong,
        ...
        public String toString() {
            return String.format("[%s] %s 题数=%d 正确=%d 积分=%d 耗时=%.1fs %s",
                    difficulty.name(), userId, total, correct, scoreEarned,
